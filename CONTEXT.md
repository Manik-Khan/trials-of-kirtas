...
## Current state (UX-audit arc — calibration UX, tabbed dock + ticker, mobile triage, dblclick chip + hints)
...
*Last updated: end of the June 12 UX-audit session — calibration UX (verified), tabbed dock + roll ticker UX-AUDIT-1/1b (verified), mobile triage + dblclick chip + first-use hints (deployed; MOBILE-TRIAGE v2 landscape fix in hand, awaiting deploy + verify). Next: verify v2 on phone portrait AND landscape, get the monster→PC "next"-bug repro, then the MOBILE AUDIT as its own mock-first session (companion layout: bottom tab bar, Feed / Map / My Character).*
