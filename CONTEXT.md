...
**⚠ KNOWN BUG (top of next): sheet can't change HP at rest + temp HP / temp max HP lost.** Diagnosed (not yet confirmed live): the **`characters` table's RLS/column-guard does NOT let a player UPDATE `vitals` on their own row.** `combatants` lets players write hp/conditions/x/y, but temp HP + hpBonus live ONLY in `characters.vitals` (never mirrored) and at rest HP lives there too — a blocked `characters.vitals` write kills both. battle.js round-trips temp/bonus correctly (`saveCombatState` sends `{hp,hpTemp,hpBonus,pipState,concentration}`; `seedSessionFromDb` reads them) and the mirror trigger preserves them — so the block is the **characters write permission**, not JS. **The live `characters` RLS isn't in the repo (dashboard/raw SQL) — can't see it.** Confirm: edit HP on the sheet with console open → `[party/vitals] save:` warning / "Save blocked — you can only edit your own character" = the rejection. **Fix to build:** a `characters` RLS update policy + column guard mirroring `combatants` (open owner/party rows; pin protected cols so players touch only `vitals`/`notes`/`inventory`, never `owner`/`key`/`structural`). M decides own-character-only vs any-party-character (shared-HUD parity).

**Files (DEPLOY PENDING — run SQL first, then push pages together):**
- Run + fold into `schema_v1.sql`: `schema_delta_vitals_hp_sync.sql`, `enable_characters_realtime.sql`, `update_liadan_portrait.sql`, `advance_turn.sql`.
- Push together (pages depend on the new module + rpc): `character-data.js`, `combatants-backend.js` (NEW), `combat.html`, `party.html`, `characters.js`.
...
**Immediate — FIX the `characters`-write block (vitals).** Write-back, full-drive vitals, Option A, and the single-source turn engine are all BUILT (see current-state). But the sheet **can't change HP at rest and loses temp HP / temp max HP** because a player can't UPDATE `vitals` on their own `characters` row. Confirm the console rejection, then ship a `characters` RLS update policy + column guard mirroring `combatants` (open owner/party rows; pin protected cols so players touch only `vitals`/`notes`/`inventory`). Prove in Postgres, fold into `schema_v1`. **Also pending: deploy this session's files** (run the 4 SQL deltas, push `character-data.js` + `combatants-backend.js` + `combat.html` + `party.html` + `characters.js` together) and run the smoke list.
