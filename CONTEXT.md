...
## Current state (Character-sheet + party-page arc — Supabase `characters`, read-path, `party.html` takeover · June 13–14)
...
**Immediate (character sheet): build the write-back layer.** `CharacterData.save(key, patch)` → owner-editable Supabase columns (RLS + guard enforce server-side; nothing else can write). This is the single gate for ALL interactivity — editable notes, HP, equip, inventory. **Start with Notes** (a textarea in STORY saving the `notes` column): smallest/safest, proves the full path end-to-end (client → Supabase → RLS → guard → git backup on next export). Then vitals/HP.
...
*Last updated: end of the June 13–14 character-sheet session — Supabase `characters` table + migration + export backstop all live (the export saga's two lessons pinned: `service_role` grants on raw-SQL tables; new keys aren't JWTs), `character-data.js` read-path, and `party.html` rebuilt as the immersive game-menu sheet (read-only v1, DEPLOYED, nav-bleed fixed, looks great). Next: the write-back layer (Notes first), then port the inventory engine into GEAR + equip slots. Housekeeping: commit the SQL / functions / `character-data.js` / `party.html`, and fold the `characters` table into `schema_v1.sql`.*
