...
const AI=require('../forge-foe-ai.js'),MonsterActor=require('../../monster-actor.js'),ForgeKitDerive=require('../forge-kit-derive.js');
...
const ctx={window:{MonsterActor,ForgeKitDerive},clog(){},escapeHtml:String,console};vm.createContext(ctx);
['rangeFromNote','foeActionFromMonster','foeActionsFromMonster','foeReferenceTile','monsterSpellFacts','foeSpellAction','foeHudFrom','foeKitFromStatblock','foeMultiattackSequence'].forEach(name=>vm.runInContext(fnSource(name),ctx));
const row={unit:'archer-1',name:'Archer 1',statblock:{name:'Archer',dex:18,ac:[16],hp:{average:16},speed:{walk:30},type:'humanoid',action:[
 ...
 {name:'Multiattack',entries:['The archer makes two attacks.']}],trait:[{name:'Keen Sight',entries:['The archer has advantage on sight checks.']}],spellcasting:[{name:'Spellcasting',headerEntries:['The archer is a 5th-level spellcaster. Its spell save DC is 13, +5 to hit with spell attacks.'],will:['{@spell guidance}','{@spell produce flame}']}],reaction:[{name:'Parry',entries:['The archer adds 2 to AC.']}]}};
...
ok(kit.actions.length===3,'production adapter exposes parsed weapons and executable combat spells');
...
ok(kit.foeHud.tabs.spells.some(x=>x.label==='guidance'&&x.reference)&&kit.foeHud.tabs.spells.some(x=>x.label==='produce flame'&&x.kind==='attack'&&x.hit===5),'enemy spell list keeps utility references and compiles supported combat spells');
...
ok(html.includes('forge-foe-ai.js?v=fai5')&&html.includes('forge-hud.js?v=b7')&&html.includes('monster-actor.js?v=ma4'),'changed modules have fresh production cache stamps');
