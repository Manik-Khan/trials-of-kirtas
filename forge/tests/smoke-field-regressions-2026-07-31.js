...
const foeTurn=source("foeTurn"),rage=source("useRage"),confirm=source("promptGodGeometryOverride"),preview=source("renderGodPreview"),bootAt=html.indexOf("async function bootSession"),rosterAt=html.indexOf("var pipelineRoster",bootAt),hydrateAt=html.indexOf("await hydrateRosterStatblocks",bootAt),liveAt=html.indexOf("await loadLiveStats()",hydrateAt);
...
ok("canonical foe refresh happens before the replay roster is constructed",bootAt>=0&&hydrateAt>bootAt&&liveAt>hydrateAt&&rosterAt>liveAt);
