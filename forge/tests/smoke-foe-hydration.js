...
["statblockHasCombatRules","rosterFoeLookupName","hydrateRosterStatblocks"].forEach(name=>vm.runInContext(fnSource(name),ctx));
(async function(){
  ...
  const unchanged=authored.statblock;await ctx.hydrateRosterStatblocks([authored],[{name:"Custom",raw:{name:"Custom",action:[{name:"Wrong"}]}}]);
  ok("complete authored statblocks remain authoritative",authored.statblock===unchanged&&authored.statblock.action[0].name==="Claw");
