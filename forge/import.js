(function () {
  ...
  var Importer = window.ForgeImageImporter;
  var Structure = window.ForgeImageStructureReview;
  var Blueprint = window.ForgeBlueprint;
  var ImageBlueprint = window.ForgeImageBlueprint;
  var ui = {};
  [
    ...
    "surfaceReceipt", "reviewJson", "openCombatProof", "cancelImport", "fatal"
  ...
  function returnKey() {
    return window.location.hash.startsWith("#return=") ? decodeURIComponent(window.location.hash.slice(8)) : "";
  }
  function openStoredHandoff(key) {
    window.location.href = "combat.html#import=" + encodeURIComponent(key);
  }
  function storeCombatHandoff() {
    ...
    var nonce = Date.now(), key = "forge-combat-blueprint-handoff:" + nonce;
    var underlayKey = key + ":artwork", blueprint, handoff;
    try {
      sessionStorage.setItem(underlayKey, ui.sourceCanvas.toDataURL("image/jpeg", 0.78));
      blueprint = ImageBlueprint.reviewToBlueprint(state.review, state.analysis, {
        sourceName: state.file.name,
        sourceRights: "User-provided local artwork",
        underlayKey: underlayKey
      });
      handoff = Blueprint.createHandoff(blueprint, { armed: false, tool: "select" });
      sessionStorage.setItem(key, JSON.stringify(handoff));
    } catch (error) {
      setStatus("This browser could not prepare the reviewed map: " + (error && error.message || "unknown error") + ". The review remains unchanged here.");
      return;
    }
    setStatus("Reviewed structures and their local artwork colors are entering the Forge.");
    openStoredHandoff(key);
  }
  ...
  ui.cancelImport.addEventListener("click", function () {
    var key = returnKey();
    window.location.href = key ? "combat.html#import=" + encodeURIComponent(key) : "combat.html";
  });
  ...
  try {
    ...
    if (!Blueprint || !Blueprint.createHandoff || !ImageBlueprint || !ImageBlueprint.reviewToBlueprint) throw new Error("The Forge Blueprint handoff did not load.");
    renderAll();
  } catch (error) {
    console.error(error);
