...
const htmlPath = new URL('../candidates/bardic-room-resilience-wave4.html', import.meta.url);
const html = fs.readFileSync(htmlPath, 'utf8');
const inlineScript = [...html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/gi)]
  .map((match) => match[1])
  .find((source) => source.trim());

if (!inlineScript) throw new Error('Wave 4 inline script was not found.');
...
vm.runInNewContext(exposed, context, { filename: 'bardic-room-resilience-wave4.inline.js' });
const api = context.window.__Wave4Test;
if (!api) throw new Error('Wave 4 test API injection failed.');
...
api.state.hostAnchor = {
  ...
  trackHash: 'hash-a',
  title: 'Track A',
  startHostMs: api.monoEpochMs() - 30000,
  positionSeconds: 5,
  duration: 120,
  receivedAt: Date.now(),
};
...
assert(readyRecovery.ready === true, 'matching bytes, verified audio, and a clock open the recovery gate');
assert(readyRecovery.positionSeconds > 38 && readyRecovery.positionSeconds < 40, 'late join predicts the active position at the future start time');

api.state.slots.a.hash = 'wrong-hash';
assert(api.recoveryReadiness().ready === false, 'a byte mismatch blocks late join');
api.state.slots.a.hash = 'hash-a';
api.state.hostAnchor.receivedAt = Date.now() - 36000;
...
assert(api.state.recoveryNeeded === false && ['joined', 'rejoined'].includes(api.state.recoveryState), 'late join clears recovery only at the scheduled start time');

api.state.samples = [{ rttMs: 12, offsetMs: 2 }];
api.markRecoveryNeeded('smoke-reset', { resetClock: true });
assert(api.state.samples.length === 0 && api.state.estimate === null, 'recovery can require a fresh listener-to-host clock');

createdSources.length = 0;
api.state.liveSources.clear();
api.ui.role.value = 'host';
...
api.state.currentHostPlayId = 'old-host-play';
api.state.currentStartHostMs = api.monoEpochMs() - 5000;
...
const publishedAnchor = api.currentHostPlaybackAnchor();
assert(publishedAnchor?.playId === 'transition-1', 'the host publishes the shared play identity');
assert(publishedAnchor?.trackHash === 'hash-b' && publishedAnchor?.slot === 'b', 'the host anchor identifies the exact active bytes');
assert(publishedAnchor?.startHostMs === transition.startHostMs && publishedAnchor?.positionSeconds === 12, 'the host anchor preserves the original start time and position');
...
console.log(`Wave 4 smoke: ${passes}/${passes} checks passed`);
