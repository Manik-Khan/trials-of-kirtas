...
if (data) {
  ok(data.contract === 'tok-quest-capture/v4-candidate', 'capture contract includes linked description mentions');
  ok(data.boundary === 'standalone-review-only-no-reads-no-writes', 'non-persistent boundary is explicit');
  ok(data.approval === 'm-approved-2026-08-25', 'M approval is recorded on the linked-description interaction');
  ...
  ok(data.descriptionMentions.syntax === '@', 'description mentions use the familiar at trigger');
  ok(data.descriptionMentions.types.join(',') === 'npc,location', 'description links only NPCs and locations');
  ok(data.descriptionMentions.storage === 'stable_entity_links', 'description mentions retain stable Living Codex identity');
  ok(data.descriptionMentions.behavior === 'hover_or_focus_tooltip', 'description links expose the accessible tooltip behavior');
  ok(data.descriptionMentions.projections.includes('right_rail') && data.descriptionMentions.projections.includes('quest_hub'), 'linked description projects wherever quest detail appears');
  ok(data.descriptionMentions.structuralFieldsRemainSeparate, 'context mentions do not replace primary quest fields');
...
has('Type <b>@</b> to link a known NPC or location', 'description teaches inline entity linking');
has('data-description-mentions', 'description owns a contained people-and-places picker');
has('data-entity-type="location" data-entity-id="tiersgard"', 'description example includes a stable location link');
has('data-entity-type="npc" data-entity-id="darius"', 'description example includes a stable NPC link');
has('Hover or focus a linked name', 'description teaches glanceable Living Codex context');
has('function insertDescriptionMention(', 'known entities can be inserted inside description prose');
has('function bindEntityTooltips(', 'capture, preview, and receipt share one tooltip binding');
has('descriptionHtml:descriptionMarkup()', 'review retains linked prose instead of flattening it');
has('Context, not bookkeeping', 'context mentions stay distinct from primary quest connections');
